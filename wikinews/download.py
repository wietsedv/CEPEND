...
parse_url = 'https://en.wikinews.org/w/api.php?action=query&format=json&prop=extracts|categories&explaintext&exlimit=1\
&pageids={}'
...
async def download_page(sem, path, pageid, title, subcats, bar):
    ...
    try:
        r = requests.get(parse_url.format(pageid))
        ...
        util.log('Downloading of {} > {} [{}] failed. retrying'.format(path, pageid, title), 'download')
